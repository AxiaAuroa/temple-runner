...
Crafty.c('Door', {
	...
	playerEntered: function() {
		...
		var currentRoom = parseInt(Crafty._current.slice(5)); // Assuming scene name is 'room-X'
		
		// Get current score
		var currentScore = Game.timeAttack.score || 0;
		
		// Call the complete level function
		Game.completeLevel(currentRoom, currentScore);
	}
