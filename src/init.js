...
var Game = {
	...
	leaderboard: {
		...
		getTopScores: function(callback) {
			// Use the local leaderboard method
			Game.leaderboard.getTopScores(callback);
		},
		
		// Save score to leaderboard
		saveScore: function(username, score) {
			// Use the local leaderboard method
			return Game.leaderboard.saveScore(username, score);
		}
	}
};
...
function gameOver(score) {
    ...
    if (playerName && playerName.trim() !== "") {
        ...
        // Save score to leaderboard
        Game.leaderboard.saveScore(playerName, score).then(result => {
            if (result.success) {
                if (result.madeLeaderboard) {
                    // Show success message for making the leaderboard
                    Crafty.e("FloatingMessage").floatingMessage("You made it to the leaderboard!");
                } else {
                    // Show message for not making the leaderboard
                    Crafty.e("FloatingMessage").floatingMessage("Score recorded, but not high enough for the leaderboard");
                }
            } else {
                // Show error message
